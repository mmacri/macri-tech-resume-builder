...
-- Fix role-related errors by modifying existing functions to use is_admin from the profiles table
-- instead of trying to check for non-existent roles like {admin,super_admin}

-- Update the is_admin function to use a simpler approach that only checks the profiles table
...
-- Update the handle_new_user function to simplify admin checks
...
BEGIN
  ...
  VALUES (
    ...
    new.email,
    -- Make the users with these emails admins by default (simplified logic)
...
-- Make sure the sync_missing_profiles function doesn't try to check non-existent roles
...
-- Run the sync_missing_profiles function to ensure all existing users have profiles
